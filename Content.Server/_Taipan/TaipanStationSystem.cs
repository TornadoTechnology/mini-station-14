using System.Linq;
using Content.Server._Taipan.Components;
using Content.Server.GameTicking;
using Content.Server.Station.Components;
using Content.Shared.GameTicking;
using Content.Shared._Taipan;
...
namespace Content.Server._Taipan;
...
public sealed class TaipanStationSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
		...
		_cfg.OnValueChanged(TaipanCCVar.TaipanEnabled, OnValueChanged);
    ...
    {
        // if (!force && (_gameTicker.RunLevel != GameRunLevel.InRound || !_cfg.GetCVar(TaipanVar.TaipanEnabled)))
        if (!_cfg.GetCVar(TaipanCCVar.TaipanEnabled))
            return;
